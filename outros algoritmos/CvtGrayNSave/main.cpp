...
//const static char* dataset_images_location = "/home/matheus-ubuntu/Desktop/sequences/00/image_1";
 const static char* dataset_images_location = "/home/matheus-ubuntu/Desktop/frames_iphone2";
...
int main(int argc, char** argv) {
  ...
  double focal = 35;
  cv::Point2d pp(640, 360); //1280x720 eh a imagem minha
  ...
  for(int numFrame=3; numFrame < MAX_FRAME; numFrame++) {
    ...
    int x = int(t_f.at<double>(0)) + 200; //coordenadas do ponto inicial x e y na visualização
    ...
    circle(traj, cv::Point(x, y), 1, CV_RGB(150, 150, 0), 2); // traj eh a matriz de zeros que vai ser recebido o desenho
    ...
    if (renderFeatures){
      ...
      for(auto point: currFeatures)
        cv::drawMarker(currImage, cv::Point(point.x,point.y), CV_RGB(0,255,0), cv::MARKER_TILTED_CROSS, 2, 1, cv::LINE_AA);
    }
